    ...
    <div class="flex flex-col gap-8">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card class="shadow-lg hover:shadow-xl transition-shadow duration-300">
          <CardHeader class="bg-gray-50 border-b">
            <CardTitle class="flex justify-between items-center">
              <p class="text-xl font-semibold text-gray-800">{{ $t('contracts.create') }}</p>
              <div class="contract-type-select w-[200px]">
                <Select v-model="selectedContractType">
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="rent">{{ $t('contracts.types.rent') }}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </CardTitle>
          ...
          <CardContent class="p-6">
            <div class="flex flex-col gap-6">
              <div class="seller-select space-y-2">
                <Label class="text-sm font-medium text-gray-700">{{ selectedContractType === 'CPCV' ? 'Vendedor' : 'Arrendatário' }}</Label>
              ...
              <div class="buyer-select space-y-2">
                <Label class="text-sm font-medium text-gray-700">{{ selectedContractType === 'CPCV' ? 'Comprador' : 'Senhorio' }}</Label>
                ...
                <Select v-model="selectedProperty">
                  ...
                  <SelectContent>
                    <SelectGroup v-if="properties.length > 0">
                      <SelectItem v-for="property in properties" :key="property.id" :value="property" class="py-2">
                        <div class="flex flex-col">
                          <span class="font-medium">[#{{ property.id }}] {{ property.district }}</span>
        ...
        <Card class="shadow-lg hover:shadow-xl transition-shadow duration-300">
          <CardHeader class="bg-gray-50 border-b">
            <CardTitle class="flex justify-between items-center">
              <h1 class="text-xl font-semibold text-gray-800">{{ $t('contracts.preview') }}</h1>
            </CardTitle>
          </CardHeader>
          <CardContent class="p-6">
            <div class="preview-container bg-white rounded-lg border">
              <div v-html="generateCPCVHtml()" ref="cpcvContainer" class="p-6 max-h-[600px] overflow-y-auto"></div>

              <template v-if="generateCPCVHtml().length === 0">
                <div class="preview-section bg-gray-50 border-t border-gray-100 rounded-b flex items-center justify-center w-full h-[284px]">
                  <div class="text-center">
                    <p class="text-gray-500 mb-2">Preencha o formulário para pré-visualizar o contrato</p>
                    <Icon name="heroicons:document-text" class="w-12 h-12 text-gray-400 mx-auto" />
                  </div>
                </div>
              </template>
            </div>

            <div v-if="generateCPCVHtml().length > 0" class="w-full flex justify-end mt-6">
              <div class="flex items-center gap-4">
                <div class="text-sm text-gray-600">
                  <p>{{ $t('contracts.price') }}: <span class="font-semibold">€{{ contractPrice }}</span></p>
                  <p v-if="!hasBalance" class="text-red-500">{{ $t('contracts.balanceWarning', [currentBalance]) }}</p>
                </div>
                <Button 
                  @click="saveContract" 
                  :disabled="!hasBalance" 
